/* ===== V06 instant controls and final visual corrections ===== */
(function () {
  "use strict";

  if (window.__ymV06InstantControlsInstalled) return;
  window.__ymV06InstantControlsInstalled = true;
  ...
  function beginInstantPaint() {
    document.documentElement.classList.add("ym-v06-instant-switch");
    document.body?.classList.add("ym-v06-instant-switch");
    requestAnimationFrame(() => requestAnimationFrame(() => {
      document.documentElement.classList.remove("ym-v06-instant-switch");
      document.body?.classList.remove("ym-v06-instant-switch");
    }));
  }

  function setPageActive(target) {
    if (!target) return;
  ...
  function syncRoleButton(adminMode) {
    const button = byId("admin-toggle");
    if (!button) return;
    const label = adminMode ? "Return to distributor workspace" : "Open Yanmar admin workspace";
    button.setAttribute("aria-pressed", String(adminMode));
    button.setAttribute("aria-label", label);
    button.setAttribute("title", label);
  }

  function switchRoleImmediately(adminMode) {
    if (!document.body) return;
    beginInstantPaint();

    const activePage = document.querySelector("main > .page.active");
    if (adminMode && activePage && activePage.id && activePage.id !== "admin") {
      lastDistributorPage = activePage.id;
    }

    document.body.classList.toggle("admin-mode", adminMode);
    document.body.classList.remove("admin-shared-tool-mode", "ym-nav-open");
    document.body.dataset.role = adminMode ? "admin" : "distributor";
    document.documentElement.dataset.role = adminMode ? "admin" : "distributor";

    const target = adminMode
      ? byId("admin")
      : (byId(lastDistributorPage) || byId("home"));
    setPageActive(target);

    syncRoleButton(adminMode);

    // Accessibility, navigation highlighting, storage and dashboard refresh happen
    // after the new role is already painted. This keeps the click response immediate.
    requestAnimationFrame(() => {
      const distributorSidebar = document.querySelector(".layout > .sidebar:not(.admin-sidebar)");
      const adminSidebar = document.querySelector("main > .admin-sidebar");
      ...
      all(".sidebar:not(.admin-sidebar) .nav-button[data-page]").forEach((button) => {
        const active = !adminMode && button.dataset.page === target?.id;
        button.classList.toggle("active", active);
        if (active) button.setAttribute("aria-current", "page");
        else button.removeAttribute("aria-current");
      });
      all(".admin-sidebar .nav-button[data-admin-target]").forEach((button) => {
        const active = adminMode && button.dataset.adminTarget === "admin";
        button.classList.toggle("active", active);
        if (active) button.setAttribute("aria-current", "page");
        else button.removeAttribute("aria-current");
      });
      all(".admin-sidebar .nav-button[data-admin-tool-target]").forEach((button) => {
      ...
      setTimeout(() => {
        if (adminMode) window.__ymInitializeAdminViews?.();
        window.YanmarAmplifyLegacy?.updateSearchIndex?.();
        window.YanmarAmplifyV17?.ensureAdminIcon?.();
      }, 0);
    });
  }
  ...
  function activateHeaderControl(target) {
    if (target.closest("#theme-toggle")) {
      toggleThemeImmediately();
      return true;
    }
    if (target.closest("#admin-toggle")) {
      switchRoleImmediately(!document.body.classList.contains("admin-mode"));
      return true;
    }
    return false;
  }
  ...
  window.addEventListener("pointerdown", (event) => {
    if (!(event.target instanceof Element) || !activateHeaderControl(event.target)) return;
  ...
  window.addEventListener("click", (event) => {
    if (!suppressHeaderClick || !(event.target instanceof Element)) return;
  ...
  window.addEventListener("keydown", (event) => {
    ...
    if (!(event.target instanceof Element) || !event.target.closest("#theme-toggle, #admin-toggle")) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    activateHeaderControl(event.target);
  }, true);
})();
...
(() => {
  ...
  const iconAssignments = [
    ...
    ['[data-admin-target="admin-support-center"]', "support"],
    ['[data-admin-target="admin-feedback-center"]', "feedback"],
  ...
  function updateSearchIndex() {
    ...
    const extraAliases = {
      assets: "brochure brochures pdf datasheet datasheets manual manuals product sheet product sheets download downloads image images visual visuals gallery social post posts template templates marketing material materials content library",
      "brochure-library": "brochure brochures pdf datasheet datasheets manual manuals document documents download downloads arabic",
      "visual-workspace": "gallery image images social post posts template templates campaign visual visuals download",
      visuals: "campaign gallery concept concepts post posts visual visuals social media",
      calendar: "content calendar planning week weeks month months schedule campaign activity activities",
      activation: "starter playbook first campaign launch checklist",
      "channel-playbook": "playbook linkedin instagram youtube email website seo social showroom channel channels",
      demo: "portal knowledge help guide instructions faq product application marketing tool",
      "website-copy": "website growth guide seo sea copy text landing page metadata keyword",
      "competitive-intelligence": "comparison compare competitor sheet builder export pdf csv",
      "tco-calculator": "tco total cost ownership lifecycle calculator fuel maintenance price pdf csv print"
    };
    searchItems = $$(selector).map((button) => {
      ...
      const aliases = `${searchAliases[target] || ""} ${extraAliases[target] || ""}`;
      const page = target ? document.getElementById(target) : null;
      const pageContent = page
        ? Array.from(page.querySelectorAll("h1,h2,h3,h4,.label,.small-red,.intro,.detail-label,a,button,[data-search-keywords]"))
          .map((node) => `${node.textContent || ""} ${node.getAttribute("data-search-keywords") || ""}`)
          .join(" ")
        : "";
      return {
        button,
        ...
        haystack: (label + " " + target + " " + aliases + " " + pageContent).toLowerCase().replace(/[^a-z0-9\u00c0-\u024f]+/gi, " ")
      };
    }).filter((item) => item.label);
  }

  function scoreSearch(item, query) {
    const normalized = query.toLowerCase().trim().replace(/[^a-z0-9\u00c0-\u024f]+/gi, " ");
    const rawTokens = normalized.split(/\s+/).filter(Boolean);
    const variants = (token) => {
      const values = [token];
      if (token.length > 4 && token.endsWith("s")) values.push(token.slice(0, -1));
      if (token.length > 5 && token.endsWith("es")) values.push(token.slice(0, -2));
      if (token.length > 5 && token.endsWith("ing")) values.push(token.slice(0, -3));
      return values;
    };
    if (!rawTokens.length) return 1;
    if (!rawTokens.every((token) => variants(token).some((variant) => item.haystack.includes(variant)))) return 0;
    const label = item.label.toLowerCase();
    let score = rawTokens.reduce((sum, token) => {
      const matched = variants(token).find((variant) => item.haystack.includes(variant)) || token;
      return sum + (label.startsWith(matched) ? 10 : label.includes(matched) ? 7 : 3);
    }, 0);
    if (label.includes(normalized)) score += 18;
  ...
  function setupSearch() {
    ...
    wrapper.innerHTML =
      '<div class="ym-global-search-shell">' +
        ...
        '<input class="ym-global-search-input" id="ym-global-search-input" type="search" autocomplete="off" aria-label="Search products, documents and tools" placeholder="Search tools, brochures, assets..." />' +
...
(function () {
  ...
  var adminKnowledge = [
    ...
    },
    {
      ...
      steps: ["Review Platform Roadmap, open risks and unresolved validation items.", "Confirm distributor/admin navigation, permissions and language behavior.", "Test campaign, library, copy, comparison, TCO, save, reopen, PDF, print, share and result-reporting flows.", "Check light/dark mode, mobile overflow and PDF output.", "Export the required evidence and record the release decision in Activity Log."],
    ...
    return `
      <div class="ym17-shell ym17-comparison-shell" data-ym17-comparison data-ym17-comparison-app data-cmp-current-step="${activeStep}">
        ...
        <section class="ym17-panel ym17-cmp-panel" data-cmp-panel="3" aria-labelledby="ym17-competitors-title">
          ...
          <div class="ym17-alert">Yanmar preset values come from the supplied product records. You may adjust them for this sheet and use Reset field to restore the original preset.</div>
        ...
        <section class="ym17-panel ym17-cmp-panel" data-cmp-panel="4" aria-labelledby="ym17-preview-title">
          <div class="ym17-panel-head">
            ...
            <div class="ym17-inline-actions">
              <button class="ym17-button" type="button" data-cmp-action="pdf">Save as PDF</button>
              <button class="ym17-button is-secondary" type="button" data-cmp-action="print">${api.icon("print")} Print</button>
    ...
    return `<table class="ym17-evidence-table">
      ...
      <tbody>${selected.map(([key, label, _recommended, unit, isCustom]) => `<tr>
        ...
        <td class="ym17-yanmar-local-cell">
          ...
          <div class="ym17-help">Preset: ${api.escapeHTML(officialPresetValue(key))}${unit && officialPresetValue(key) !== missing() ? ` ${api.escapeHTML(unit)}` : ""}${isCustom ? ' <span class="ym17-badge">Manually added</span>' : ""}</div>
  ...
  function previewMarkup(imageOverrides = null) {
    ...
    return `<article class="ym17-export-sheet" style="--ym17-columns:${productColumns}" data-ym17-export-sheet>
      ...
      <table class="ym17-export-table">
        ...
        <tbody>${selected.map(([key, label, _recommended, unit, isCustom]) => `<tr><th>${api.escapeHTML(label)}${unit ? ` (${api.escapeHTML(unit)})` : ""}</th><td class="is-yanmar">${api.escapeHTML(officialValue(key))}${isCustom ? '<br><small>Manually added</small>' : ""}</td>${competitors.map((competitor) => `<td>${api.escapeHTML(competitor.values[key] || "Not available")}</td>`).join("")}</tr>`).join("")}</tbody>
      </table>
...
<body>
  ...
  async function pdfExport() {
    ...
    try {
      const canvas = await renderCanvas();
      if (!window.YanmarCanvasPdf?.download) throw new Error("PDF exporter unavailable");
      await window.YanmarCanvasPdf.download(canvas, `${comparisonFileBase()}.pdf`);
      api.track("Downloaded comparison PDF", state.title, state.category);
      api.notify("Comparison PDF downloaded.");
    } catch (error) {
      console.error("Comparison PDF export failed", error);
      api.notify("The PDF could not be generated. Try again after checking product images.", "danger");
    } finally {
      exportBusy = false;
    }
  }
...
(function () {
  ...
  function mountAdmin() {
    adminSection = api.byId("admin-support-center");
    if (!adminSection) return;
    adminSection.innerHTML = adminMarkup();
  ...
  function updateBadge() {
    ...
    const unread = items.filter((item) => !requestMeta(item.id).read).length;
    const open = items.filter((item) => requestMeta(item.id).status !== "Completed").length;
    const distributorOpen = items.filter((item) => item.distributor === activeDistributor() && requestMeta(item.id).status !== "Completed").length;
    const badgeCounts = {
      "admin-nav-support-count": open,
      "nav-support-count": distributorOpen,
    };
    Object.entries(badgeCounts).forEach(([id, count]) => {
      const badge = api.byId(id);
      if (!badge) return;
      badge.hidden = count === 0;
      badge.textContent = count > 99 ? "99+" : String(count);
    });
    const overviewCount = api.byId("admin-support-count");
    if (overviewCount) overviewCount.textContent = String(items.length);
    const adminOverviewBadge = api.byId("admin-nav-inbox-count");
    if (adminOverviewBadge && unread > 0) {
      const current = Number(adminOverviewBadge.textContent || 0);
      adminOverviewBadge.hidden = false;
      adminOverviewBadge.textContent = String(Math.min(99, Math.max(current, unread)));
    }
  }
...
(function () {
        ...
        <section class="ym17-panel" aria-labelledby="tco-results-title">
          <div class="ym17-panel-head">
            ...
            <div class="ym17-inline-actions">
              ...
              <button class="ym17-button" type="button" data-tco-action="pdf">Save as PDF</button>
              <button class="ym17-button is-secondary" type="button" data-tco-action="print">${api.icon("print")} Print</button>
            </div>
          </div>
  ...
  async function pdfExport(button) {
    if (!calculations.length && !calculate()) return;
    const original = button?.textContent || "Save as PDF";
    try {
      if (button) { button.disabled = true; button.textContent = "Creating PDF..."; }
      const canvas = renderTcoCanvas();
      if (!window.YanmarCanvasPdf?.download) throw new Error("PDF exporter unavailable");
      await window.YanmarCanvasPdf.download(canvas, `${api.slug(state.title)}.pdf`);
      api.track("Downloaded TCO PDF", state.title, state.category);
      api.notify("TCO PDF downloaded.");
    } catch (error) {
      console.error("TCO PDF export failed", error);
      api.notify("The TCO PDF could not be created. Try again after calculating the sheet.", "danger");
    } finally {
  ...
  function handleAction(button) {
    ...
    if (action === "csv") csvExport();
    if (action === "pdf") pdfExport(button);
    if (action === "print") printExport();
...
(function () {
  ...
  const KNOWLEDGE_ITEMS = [
    ...
    },
    {
      ...
      guidance: ["Enter brand and model first.", "Add only facts supported by a source.", "Use up to four independent competitors.", "Review Not available values before publication.", "Download HTML or CSV, use Save as PDF for a direct PDF file, or use Print for a paper copy."],
...
/* Direct canvas-to-PDF download helper. Creates a real PDF without opening the browser print dialog. */
(function () {
  "use strict";
  if (window.YanmarCanvasPdf?.download) return;
  ...
  const objectBytes = (parts) => Array.isArray(parts) ? combine(parts) : encode(parts);

  async function createPdf(canvas, quality = 0.92) {
    const pageWidth = 841.89;
    const pageHeight = 595.28;
    const pagePixelHeight = Math.max(1, Math.round(canvas.width * pageHeight / pageWidth));
    const pages = [];
    for (let sourceY = 0; sourceY < canvas.height; sourceY += pagePixelHeight) {
      ...
      pages.push({ width: pageCanvas.width, height: pageCanvas.height, bytes: jpegBytes(pageCanvas.toDataURL("image/jpeg", quality)) });
    }

    const objects = [];
    const pageIds = pages.map((_page, index) => 3 + index * 3);
    objects[0] = objectBytes("<< /Type /Catalog /Pages 2 0 R >>");
    objects[1] = objectBytes(`<< /Type /Pages /Kids [${pageIds.map((id) => `${id} 0 R`).join(" ")}] /Count ${pages.length} >>`);
    pages.forEach((page, index) => {
      const pageId = 3 + index * 3;
      const contentId = pageId + 1;
      const imageId = pageId + 2;
      objects[pageId - 1] = objectBytes(`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${pageWidth} ${pageHeight}] /Resources << /XObject << /Im${index + 1} ${imageId} 0 R >> >> /Contents ${contentId} 0 R >>`);
      const content = encode(`q\n${pageWidth} 0 0 ${pageHeight} 0 0 cm\n/Im${index + 1} Do\nQ`);
      objects[contentId - 1] = objectBytes([encode(`<< /Length ${content.length} >>\nstream\n`), content, encode("\nendstream")]);
      objects[imageId - 1] = objectBytes([
    ...
    return new Blob([combine(chunks)], { type: "application/pdf" });
  }

  async function download(canvas, filename) {
    const blob = await createPdf(canvas);
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = String(filename || "yanmar-comparison.pdf").replace(/[^a-z0-9._-]+/gi, "-");
    anchor.rel = "noopener";
  ...
  window.YanmarCanvasPdf = { createPdf, download };
}());
...
(function () {
  ...
  function renderBuilder() {
    ...
    root.dataset.built = "COMPARISON-BUILT-20260723-07";
    root.innerHTML = `
      <section class="ym21-hero">
        <div>
      ...
      <section class="ym21-panel">
        ...
        <div class="ym21-actions ym21-export-actions">
          <button type="button" class="ym21-btn primary" data-ym21-action="pdf">Save as PDF</button>
          <button type="button" class="ym21-btn" data-ym21-action="html">Download comparison sheet</button>
          <button type="button" class="ym21-btn" data-ym21-action="csv">Download Excel-ready CSV</button>
          <button type="button" class="ym21-btn" data-ym21-action="print">Print</button>
  ...
  function fact(key, label) {
    ...
    return `<div class="ym21-fact"><strong>${esc(label)}</strong><label class="ym21-field"><input data-ym21-yvalue="${esc(key)}" value="${esc(clean(state.yanmar[key]))}" placeholder="${NA}"></label><div><button type="button" class="ym21-btn" data-ym21-action="reset-field" data-key="${esc(key)}">Reset field</button></div></div>`;
  }
  ...
  function sheet() {
    ...
    return `<article class="ym21-sheet">
      ...
      <div class="ym21-table-wrap"><table class="ym21-table"><thead><tr><th>Specification</th><th>Yanmar</th>${comps.map((item) => `<th>${esc(nameOf(item))}</th>`).join("")}</tr></thead><tbody>${fields().map(([key, label]) => `<tr><th>${esc(label)}</th><td>${esc(clean(state.yanmar[key]))}</td>${comps.map((item) => `<td>${esc(clean(item.values[key]))}</td>`).join("")}</tr>`).join("")}</tbody></table></div>
  ...
  function wrapPdfCanvasText(context, text, maxWidth) {
  ...
  function loadPdfCanvasImage(url) {
    return new Promise((resolve) => {
      const source = clean(url);
      if (source === NA) return resolve(null);
      const img = new Image();
      let done = false;
      const finish = (value) => {
        if (done) return;
  ...
  async function renderComparisonPdfCanvas() {
    const comps = state.competitors.slice(0, 4);
    const specs = fields();
    const productsForPdf = [
      { name: `Yanmar ${current().id}`, note: "Reference product", image: state.yImgData || state.yImgUrl || current().image, yanmar: true },
      ...comps.map((item) => ({ name: nameOf(item), note: clean(item.source), image: item.imgData || item.imgUrl, yanmar: false })),
    ];
    const sourceLines = [
      "Yanmar source: existing Yanmar Amplify product information. Verify all claims against the latest local datasheet before publishing.",
      ...comps.map((item) => `${nameOf(item)} - Source: ${clean(item.source)}. Notes: ${clean(item.notes)}.`),
    ];
    const width = 1600;
    const rowHeight = 76;
    const sourceHeight = Math.max(150, sourceLines.length * 54 + 48);
    const height = 190 + 220 + specs.length * rowHeight + sourceHeight;
    const canvas = document.createElement("canvas");
    canvas.width = width;
    ...
    ctx.fillStyle = "#ed0016";
    ctx.fillRect(0, 0, width, 12);

    ctx.fillStyle = "#ed0016";
    ctx.font = "900 18px Arial";
    ctx.fillText(String(cats[state.category].label || "YANMAR COMPARISON").toUpperCase(), 58, 52);
    ctx.fillStyle = "#111827";
    ctx.font = "800 42px Arial";
    wrapPdfCanvasText(ctx, state.title, 1120).slice(0, 2).forEach((line, index) => ctx.fillText(line, 58, 102 + index * 45));
    ctx.fillStyle = "#ed0016";
    ctx.font = "italic 900 36px Arial";
    ctx.textAlign = "right";
    ctx.fillText("YANMAR", width - 58, 76);
    ctx.textAlign = "left";
    ctx.fillStyle = "#566276";
    ctx.font = "18px Arial";
    ctx.fillText(`${clean(state.useCase)} - ${clean(state.market)} - ${new Date().toLocaleDateString()}`, 58, 168);

    const cardY = 196;
    const gap = 14;
    const cardWidth = (width - 116 - gap * (productsForPdf.length - 1)) / productsForPdf.length;
    const loadedImages = await Promise.all(productsForPdf.map((item) => loadPdfCanvasImage(item.image)));
    productsForPdf.forEach((item, index) => {
      const x = 58 + index * (cardWidth + gap);
      ctx.fillStyle = item.yanmar ? "#fff3f5" : "#f8fafc";
      ctx.fillRect(x, cardY, cardWidth, 190);
      ctx.strokeStyle = item.yanmar ? "#ed0016" : "#d9e0ea";
      ctx.lineWidth = item.yanmar ? 2 : 1;
      ctx.strokeRect(x, cardY, cardWidth, 190);
      const img = loadedImages[index];
      if (img) {
        const scale = Math.min((cardWidth - 28) / img.width, 105 / img.height);
        const drawWidth = img.width * scale;
        const drawHeight = img.height * scale;
        ctx.drawImage(img, x + (cardWidth - drawWidth) / 2, cardY + 12 + (105 - drawHeight) / 2, drawWidth, drawHeight);
      } else {
        ctx.fillStyle = "#64748b";
        ctx.font = "15px Arial";
        ctx.fillText("Image not available", x + 16, cardY + 66);
      }
      ctx.fillStyle = "#111827";
      ctx.font = "700 18px Arial";
      wrapPdfCanvasText(ctx, item.name, cardWidth - 24).slice(0, 2).forEach((line, lineIndex) => ctx.fillText(line, x + 12, cardY + 140 + lineIndex * 20));
      ctx.fillStyle = "#64748b";
      ctx.font = "13px Arial";
      wrapPdfCanvasText(ctx, item.note, cardWidth - 24).slice(0, 1).forEach((line) => ctx.fillText(line, x + 12, cardY + 180));
    });

    const tableX = 58;
    const tableY = cardY + 214;
    const labelWidth = 250;
    const valueColumns = productsForPdf.length;
    const valueWidth = (width - 116 - labelWidth) / valueColumns;
    specs.forEach(([key, label], rowIndex) => {
      const y = tableY + rowIndex * rowHeight;
      ctx.fillStyle = rowIndex % 2 ? "#ffffff" : "#f8fafc";
      ctx.fillRect(tableX, y, width - 116, rowHeight);
      ctx.strokeStyle = "#d9e0ea";
      ctx.lineWidth = 1;
      ctx.strokeRect(tableX, y, width - 116, rowHeight);
      ctx.fillStyle = "#111827";
      ctx.font = "700 15px Arial";
      wrapPdfCanvasText(ctx, label, labelWidth - 22).slice(0, 3).forEach((line, lineIndex) => ctx.fillText(line, tableX + 12, y + 25 + lineIndex * 17));
      const values = [clean(state.yanmar[key]), ...comps.map((item) => clean(item.values[key]))];
      values.forEach((value, columnIndex) => {
        const cellX = tableX + labelWidth + columnIndex * valueWidth;
        ctx.strokeRect(cellX, y, valueWidth, rowHeight);
        if (columnIndex === 0) {
          ctx.fillStyle = "#fff3f5";
          ctx.fillRect(cellX + 1, y + 1, valueWidth - 2, rowHeight - 2);
        }
        ctx.fillStyle = columnIndex === 0 ? "#d60018" : "#111827";
        ctx.font = columnIndex === 0 ? "700 14px Arial" : "14px Arial";
        wrapPdfCanvasText(ctx, value, valueWidth - 20).slice(0, 3).forEach((line, lineIndex) => ctx.fillText(line, cellX + 10, y + 23 + lineIndex * 17));
      });
    });

    let sourceY = tableY + specs.length * rowHeight + 36;
    ctx.fillStyle = "#111827";
    ctx.font = "800 17px Arial";
    ctx.fillText("Sources and notes", 58, sourceY);
    sourceY += 30;
    ctx.fillStyle = "#566276";
    ctx.font = "14px Arial";
    sourceLines.forEach((source) => {
      const lines = wrapPdfCanvasText(ctx, source, width - 116).slice(0, 3);
      lines.forEach((line, index) => ctx.fillText(line, 58, sourceY + index * 18));
      sourceY += Math.max(32, lines.length * 18 + 14);
    });
    return canvas;
  }

  async function exportComparisonPdf(button) {
    const original = button?.textContent || "Save as PDF";
    if (button) {
      button.disabled = true;
    ...
    try {
      const canvas = await renderComparisonPdfCanvas();
      if (!window.YanmarCanvasPdf?.download) throw new Error("PDF exporter unavailable");
      await window.YanmarCanvasPdf.download(canvas, `${fileName(state.title)}.pdf`);
      notify("Comparison PDF downloaded.");
    } catch (error) {
      console.error("Comparison PDF export failed", error);
      notify("The PDF could not be created. Check the product images and try again.");
    } finally {
      if (button) {
        button.disabled = false;
        button.textContent = original;
  ...
  function download(name, content, type) {
    try {
      const payload = type.includes("csv") ? `\ufeff${content}` : content;
      ...
      notify(`${name} downloaded. Check your Downloads folder.`);
    } catch (error) {
      console.error("Comparison download failed", error);
      notify("The download could not start. Allow downloads for this site and try again.");
    }
  }
